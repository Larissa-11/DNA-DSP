...
class CustomRvContinuous(rv_continuous):
    ...
    def multithreaded_rvs(size, pdf_str, min_value, max_value):
        ...
        for i in range(cpu_count - 1):
            async_results.append(pool.apply_async(CustomRvContinuous.create_portion_sample_rvs,
                                                  (pdf_str, min_value, max_value, int(size / cpu_count))))
        async_results.append(pool.apply_async(CustomRvContinuous.create_portion_sample_rvs, (
            pdf_str, min_value, max_value, size - (cpu_count - 1) * int(size / cpu_count))))
