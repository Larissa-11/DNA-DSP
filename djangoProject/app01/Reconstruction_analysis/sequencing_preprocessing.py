...
def write_fasta_new(final_clusters, reads, sequencing_sorted_file, output_file):
    sequences_index_dict = {}
    with open(sequencing_sorted_file, 'r') as file:
        i = 0
        for line in file:
            line = line.rstrip()
            if line.startswith('>'):
    ...
    with open(output_file, 'w') as file:
        for i, cluster in enumerate(final_clusters):
            ...
            for seq in cluster:
                # print(seq, reads[seq])
                file.write(f'>Seq{sequences_index_dict[seq]}_Cluster{i + 1}\n')
                file.write(f'{reads[seq]}\n')


if __name__ == '__main__':
    input_file = "../datasets/test_data/test_cluster_50_with_labels.fasta"
    target_length = 110  # 指定的长度
